...
class MaskedTimeseries(NamedTuple):
    """
    Estrutura de dados para entrada de séries temporais mascaradas no modelo Toto.
    Usada em main.py para empacotar os dados de entrada antes de passar ao forecaster/modelo.

    Campos:
        series: Tensor dos valores das séries temporais (shape: batch, variates, series_len)
        padding_mask: Máscara booleana indicando onde há dados válidos
        id_mask: Máscara de identificação (útil para múltiplas séries)
        timestamp_seconds: Timestamps em segundos desde epoch
        time_interval_seconds: Intervalo médio entre timestamps
    """
