# Este arquivo foi movido para inference/forecaster.py. Remova qualquer referência antiga.
...
from data.util.dataset import MaskedTimeseries, pad_array, pad_id_mask, replace_extreme_values
...
class TotoForecaster:
    """
    Classe responsável por realizar a previsão (forecast) usando o modelo Toto.
    Usada em main.py para gerar previsões a partir de dados históricos, utilizando decodificação autoregressiva.

    Esta classe encapsula o modelo TotoBackbone e fornece métodos para gerar previsões para séries temporais.
    O método forecast é chamado em main.py para prever os próximos passos da série.

    Processo de previsão:
    1. Processa o contexto de entrada (dados históricos)
    2. Para cada passo futuro, gera uma distribuição de valores possíveis
